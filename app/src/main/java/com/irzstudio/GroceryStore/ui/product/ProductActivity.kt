...
class ProductActivity : AppCompatActivity() {
    ...
    private fun addProductToCart(productEntity: ProductEntity) {
        if (productEntity.qty == 0) {
            ...
            Toast.makeText(applicationContext, "Продукт добавлен в корзину", Toast.LENGTH_SHORT).show()
        } else {
            viewModel.removeProduct(productEntity)
            Toast.makeText(applicationContext, "Продукт удален из корзины", Toast.LENGTH_SHORT).show()
        }
    }
}